...
class CM_Error {
	 ...
	 * @return CM_Error
	 */
	function CM_Error($code = '', $message = '', $data = '') {
		if ( empty($code) )
			return;
...
/**
 * Check whether variable is a CCM Error.
 *
 * Looks at the object and if a CM_Error class. Does not check to see if the
 * parent is also CM_Error, so can't inherit CM_Error and still use this
 * function.
 *
 * @since 1.0
 *
 * @param mixed $thing Check if unknown variable is CCM Error object.
 * @return bool True, if CM_Error. False, if not CM_Error.
 */
function is_cm_error($thing) {
	if ( is_object($thing) && is_a($thing, 'CM_Error') )
		return true;
	return false;
}
?>